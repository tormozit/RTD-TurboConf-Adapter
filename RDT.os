...
//
Процедура ПерейтиВПозицию(Знач НомерКолонки = 0, Знач НомерСтроки = 0, Знач ПолеТекстаПрограммы = Неопределено, Знач ЛиПереходКСоседнемуПараметру = Ложь)
	...
	ИначеЕсли ПолеТекстаПрограммы <> Неопределено Тогда 
		Если Не ЛиПереходКСоседнемуПараметру Тогда
			ВыделениеДвумерное = ПолеТекстаПрограммы.ПолеТекста.ВыделениеДвумерное();
		...
		КонецЕсли;
		ВыделениеОдномерное = ПолеТекстаПрограммы.ПолеТекста.ВыделениеОдномерное();
		РазвернутьБлокМодуля(ВыделениеОдномерное.Начало - 1, ВыделениеОдномерное.Конец - 1,, ЛиПереходКСоседнемуПараметру);
	Иначе 
		Позиция = 0; 
		Пустышка = 0;
...
Процедура НайтиПоследнююПравкуВХранилище()
	...
	Пока Истина Цикл
		...
		КонецЕсли;
		ТурбоКонф.Ждать(50);
		ТекстСтрокиИсторииНовый = ВыделенныйТекст();
...
Процедура СообщитьОПроблеме()
	...
	ИмяФайлаОшибка = ПутьКФайлам + "\Ошибка.txt";
	...
	ЗаписьЗип.Добавить(ИмяФайлаМодуля, РежимСохраненияПутейZIP.НеСохранятьПути);
	ЗаписьЗип.Добавить(ИмяФайлаИнфо, РежимСохраненияПутейZIP.НеСохранятьПути);
	ЗаписьЗип.Записать();
