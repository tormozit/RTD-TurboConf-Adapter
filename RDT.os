...
Функция ПерейтиПоСсылкеСтрокиМодуля(Знач СсылкаСтроки = "", Знач ИмяРасширения = "")
	...
	Если ЭтоПолноеИмяМД(МодулиИР, СсылкаСтроки) Тогда
		...
		Если Успех и ЗначениеЗаполнено(ПутьКЭлементуФормы) Тогда
			ТурбоКонф.Клавиша(Клавиши.Enter);
	...
	Если ТурбоКонф.ВосстановитьЗначениеСеанса(КлючХранения) = Неопределено Тогда
		...
		Если ТекущаяВерсияТурбоКонф > "6.1.8855.39060" Тогда
			...
			Если ТурбоКонф.ПолучитьЗначениеНастройки("T9AssistSmartFieldsEnabled") Тогда
				СписокРекомендаций.Добавить("Отключить настройку Турбоконф ""Редактирование/ Подсказывать имена колонок""");
			КонецЕсли; 
			Если Не ТурбоКонф.ПолучитьЗначениеНастройки("T9AlwaysShowOnCtrlSpace") Тогда
				СписокРекомендаций.Добавить("Включить настройку Турбоконф ""Разработчикам/ Вседа показывать подсказку Т9 вместо штатной подсказки""");
			КонецЕсли; 
			...
			Если ЗначениеНастройки = "1" Тогда
				СписокРекомендаций.Добавить("Отключить настройку конфигуратора ""Модули/ Редактирование/ Автоматическая подсказка при наборе '.'""");
			КонецЕсли;
			Если СписокРекомендаций.Количество() > 0 Тогда
				ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), "Рекомендуется:" + Символы.ПС + "- " + СтрСоединить(СписокРекомендаций, "- "), 10000);
			КонецЕсли;
		КонецЕсли;
		ТурбоКонф.АктивироватьОкно(Конфигуратор.ОкноВызоваСкрипта);
	КонецЕсли;
