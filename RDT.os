...
//@script_hotkey None            ОткрытьСписок Открыть список
...
//@_script_hotkey Control+F       НайтиСтроку Найти строку
...
// Окно "Выражение"
//@_button {'Name': 'ОтладитьОбъект', 'Text': '', 'Image': 'Картинки\\ирОтладка.gif', 'Anchor': 'RightTop', 'Offset': '-184,50', 'Width': 16, 'Height': 16, 'BackColor': '', 'Filter': '^Выражение$' }
//@button {'Name': 'ОтладитьОбъект', 'Text': 'Отладить объект', Hint:'Отладить объект в подходящем инструменте ИР', 'Image': '', 'Anchor': 'RightTop', 'Offset': '-190,50', 'Width': 95, 'Height': 18, 'BackColor': '', 'Filter': '^Выражение$' }
//@button {'Name': 'ВыполнитьПроизвольныйКод', 'Text': 'Выполнить код', Hint:'Откроется редактор кода ИР и после нажатия в нем ОК код будет выполнен', 'Image': '', 'Anchor': 'RightTop', 'Offset': '-410,50', 'Width': 90, 'Height': 18, 'BackColor': '', 'Filter': '^Выражение$' }
//@_button {'Name': 'РедактироватьТекст', 'Text': '', 'Tooltip': 'Редактировать текст', 'Image': 'Картинки\\ирПолеТекстовогоДокумента.gif', 'Anchor': 'RightTop', 'Offset': '-290,50', 'Width': 16, 'Height': 16, 'BackColor': '', 'Filter': '^Выражение$' }
//@button {'Name': 'РедактироватьТекст', 'Text': 'Редактировать текст', Hint:'Редактировать многострочный текст в редакторе текста ИР', 'Image': '', 'Anchor': 'RightTop', 'Offset': '-290,50', 'Width': 115, 'Height': 18, 'BackColor': '', 'Filter': '^Выражение' }
// Окно "Параметры точки останова"
//@button {'Name': 'ВставитьУсловниеИзТабло', 'Text': 'Вставить из табло', Hint:'Вставить условие из уже выделенных в табло выражений', 'Image': '', 'Anchor': 'RightTop', 'Offset': '-150,50', 'Width': 100, 'Height': 18, 'BackColor': '', 'Filter': '^Параметры точки останова$' }
// Окно "Редактирование типа данных"
//@button {'Name': 'ФильтрИзСинонима', 'Text': 'Фильтр из синонима', Hint:'Установить фильтр из слов синонима свойства, для которого выбирается тип', 'Image': '', 'Anchor': 'LeftTop', 'Offset': '160,4', 'Width': 115, 'Height': 18, 'BackColor': '', 'Filter': '^Редактирование типа данных$' }
// Окно "Процедуры и функции"
//@button {'Name': 'СписокОбщихМетодов', 'Text': 'Общие методы', Hint:'Список общих методов ИР', 'Image': '', 'Anchor': 'RightTop', 'Offset': '-5,150', 'Width': 85, 'Height': 18, 'BackColor': '', 'Filter': '^Процедуры и функции$' }
//@_button {'Name': 'СписокОбщихМетодов', 'Text': 'Общие методы', Hint:'Список общих методов ИР', 'Image': '', 'Anchor': 'RightTop', 'Offset': '-5,150', 'Width': 85, 'Height': 18, 'BackColor': '', 'Filter': '^JumpToForm$' }
//@_button {'Name': 'ПерейтиКОпределению', 'Text': 'Перейти ИР', Hint:'Перейти к определению ИР', 'Image': '', 'Anchor': 'RightTop', 'Offset': '-40,-20', 'Width': 85, 'Height': 18, 'BackColor': '', 'Filter': '^Объект перехода$' }
...
Процедура УстановитьФильтрТипаИзСинонимаРеквизита() Экспорт
	...
	Для СчетчикШагов = 1 По 6 Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Для Счетчик = 1 По СчетчикШагов Цикл
...
Процедура АктивироватьСтрокуДерева(Знач Путь) Экспорт
	...
	Для Каждого Фрагмент Из Путь Цикл
		ТурбоКонф.КликнутьПоЭлементу(Фрагмент, ИмяТипаЭлемента, 0, Ложь);
...
Процедура ЗавершитьВызовВнешнегоОкнаАсинх(Знач ОкноВозвратаФокуса = Неопределено, Знач ИдентификаторПроцесса = 0) Экспорт
...
Функция ПерейтиПоСсылкеСтрокиМодуля(Знач СсылкаСтроки = "", Знач ИмяРасширения = "")
	...
	Если ЭтоПолноеИмяМД(МодулиИР, СсылкаСтроки) Тогда
		...
		ИначеЕсли Найти(СсылкаСтроки, МаркерСтандартный) > 0 Тогда
			Фрагменты = МодулиИР.ирОбщий.СтрРазделитьЛкс(СсылкаСтроки, МаркерСтандартный);
		...
		ИначеЕсли СтрНачинаетсяС(СсылкаСтроки, "ПакетXDTO.") Тогда
			...
			СсылкаСтроки = МодулиИР.ирОбщий.СтрокаБезПоследнегоФрагментаЛкс(СсылкаСтроки);
		КонецЕсли;
		Если ЗначениеЗаполнено(ИмяРасширения) Тогда
		...
		Если ЗначениеЗаполнено(ПутьКЭлементуФормы) Тогда
			...
			Если СтрНачинаетсяС(ПутьКЭлементуФормы, МаркерЭлементаФормы) Тогда
				...
				ИмяТекущегоЭлемента = НайтиУзелДерева(ПодключениеИР, ИмяЭлементаФормы);
			КонецЕсли;
		...
		ИначеЕсли ЗначениеЗаполнено(ИмяЭлементаМакета) Тогда
			...
			Если ТурбоКонф.ПолучитьЗаголовокТекущегоОкна() = "Имена" Тогда
				...
				ТурбоКонф.КликнутьПоЭлементу("Выбрать", "кнопка", 0, Ложь);
				ТурбоКонф.Клавиша(Клавиши.Escape);
			КонецЕсли;
		ИначеЕсли ЗначениеЗаполнено(ИмяТипаХДТО) Тогда
			ИмяТекущегоЭлемента = НайтиУзелДерева(ПодключениеИР, ИмяТипаХДТО, Истина);
		КонецЕсли;
		Возврат Истина; 
	КонецЕсли;
...
Функция НайтиУзелДерева(Знач ПодключениеИР, Знач ТекстИскомогоЭлемента, Знач СвернутьДерево = Ложь) Экспорт
	...
	ТурбоКонф.Клавиша(Клавиши.Home);
	ТурбоКонф.КонтролШифтКлавиша(Клавиши.Add);
	НайтиСтрокуВОкне(ТекстИскомогоЭлемента);
	ИмяТекущегоЭлемента = ТурбоКонф.ПолучитьИмяЭлемента();
	Сообщить("ИмяТекущегоЭлемента=" + ИмяТекущегоЭлемента);
	Если ИмяТекущегоЭлемента = ТекстИскомогоЭлемента Тогда
		ТурбоКонф.Клавиша(Клавиши.Escape); 
	Иначе 
		// Т.к. поиск делается по подстроке, то может быть найдено не то, что нужно. Поэтому оставляем дерево открытым.
		ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), "Если найден неверный элемент, нажмите F3 для поиска следующего варианта");
...
Функция ОткрытьСписок(Знач ЛиМодально = Ложь)
	ТипЭлементаИнтерфейса = ПолучитьТипТекущегоЭлемента();
	...
	ЗаголовокОкна = ТурбоКонф.ПолучитьЗаголовокТекущегоОкна();
	Сообщить("заголовок окна = " + ЗаголовокОкна);
	Если ЗаголовокОкна = "Закладки" Тогда
		ЧислоКолонок = 4;
	ИначеЕсли ЗаголовокОкна = "Точки останова" Тогда
		ЧислоКолонок = 5;
	ИначеЕсли ЗаголовокОкна = "Предметы отладки" Тогда
		ЧислоКолонок = 5;
	КонецЕсли;
	// Для таблиц работает некорректно - теряются повторяющиеся тексты https://turboconf.ru/Tasks/9743
	//Если ЛиТаблица Тогда
	//	ТекстыПолей = ТурбоКонф.ПолучитьТекстТаблицы(); // Неопределено
	//Иначе
		ТекстыПолей = ТурбоКонф.ПолучитьЭлементыСписка();
	//КонецЕсли;
	...
	Иначе
		// В списке глобальных команд управляемой формы
	КонецЕсли;
	ИндексВыбранногоЭлемента = ВыбратьЭлементСписка(ТекстыПолей, ТекстТекущейЯчейки, ЛиМодально, ЛиТаблица, ЧислоКолонок);
...
//.
// Параметры:
//    Элементы - Массив Из Строка - 
//    ТекстТекущейЯчейки - ? - 
//    ЛиМодально - Булево - 
//    ЛиТаблица - ПеречислениеСсылка.ТипЭлементаИнтерфейса - 
//    ЧислоКолонок - Число - 
//    ИндексВыбранногоЭлемента - Число, Неопределено - 
Функция ВыбратьЭлементСписка(Знач Элементы, Знач ТекстТекущейЯчейки = "", Знач ЛиМодально = Истина, Знач ЛиТаблица = Ложь, Знач ЧислоКолонок = Неопределено, Знач Заголовок = "") Экспорт
	ПодключениеИР = ПодключениеИР();
	Если ПодключениеИР = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	МодулиИР = МодулиИР(ПодключениеИР);
	ТекстыПолейСтрока = ОбъектВЖСОН(Элементы);
	ИдентификаторПроцессаИР = МодулиИР.ирКэш.ИдентификаторПроцессаОСЛкс();
	Окно = ТурбоКонф.ПолучитьАктивноеОкно();
	Если ЛиМодально Тогда
		ПодключениеИР.Visible = Истина;
		ОповеститьПользователяОПриостановке();
		//ТурбоКонф.НачатьВызовВнешнегоОкнаАсинх(".*", 3000, Конфигуратор.ГлавноеОкно, РежимОткрытияВнешнегоОкна.Диалог, Истина, ИдентификаторПроцессаИР); // Так окно не получает фокус
		ТурбоКонф.НачатьВызовВнешнегоОкнаАсинх("(Список|Таблица|Введи).*", 3000, Окно, РежимОткрытияВнешнегоОкна.Диалог, Истина, ИдентификаторПроцессаИР);
	Иначе  
		ВосстановитьОкноПриложения(ПодключениеИР);
	...
	КонецЕсли;
	Попытка
		ИндексВыбранногоЭлемента = МодулиИР.ирКлиент.ОткрытьТаблицуИзМассиваИзСтрокиВнутрЛкс(ТекстыПолейСтрока, ТекстТекущейЯчейки, ЛиТаблица, ЛиМодально, ЧислоКолонок, Заголовок);
	Исключение
		Если ЛиМодально Тогда
			ОписаниеОшибки = ОписаниеОшибки();
			ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), ОписаниеОшибки);
...
Процедура НайтиВСписке() 
	ИндексВыбранногоЭлемента = ОткрытьСписок(Истина);
	Если ИндексВыбранногоЭлемента <> Неопределено Тогда
		ТурбоКонф.Клавиша(Клавиши.Home);
		НажатьКлавишуЦикл(Клавиши.Down, ИндексВыбранногоЭлемента);
	КонецЕсли; 
КонецПроцедуры  

// Неуспешая попытка сделать замену поиска в окне Дополнительно
Процедура НайтиСтроку() 
...
Функция ПолеТекстаПрограммыИР(Знач ПодключениеИР, Знач ТекстПоля = "", Знач Позиция0Поле = 0, Знач ВыделенныйТекст = "", ПереходитьВоВложенныйКонтекст = Истина, Знач КакВызовМетода = Неопределено,
	...
	Если ЗначениеЗаполнено(ПозицияВТексте) И ЯзыкПрограммы = 0 И ЗначениеЗаполнено(ПолноеИмяМодуля) Тогда
		...
		Пока ПоследниеМетоды.Количество() > 20 Цикл
...
Процедура ОбъявитьТипВыражения()
	...
	//ТурбоКонф.КонтролШифтКлавиша(Клавиши.F10);
	//ТурбоКонф.КликнутьПоЭлементу("Рефакторинг", "элемент меню", 0, Ложь);
	...
	//ТурбоКонф.КликнутьПоЭлементу(ИмяЭлементаМеню, "элемент меню", 0, Ложь);
...
Процедура СинтаксПомощник()
	...
	////	ТурбоКонф.ЖдатьСменуТекущегоЭлемента(СтарыйИД, 1000, Истина);
	////КонецЕсли;
	//ТурбоКонф.КликнутьПоЭлементу("Индекс", "элемент вкладки", 0, Ложь);
	//ТурбоКонф.ЖдатьСменуТекущегоЭлемента(СтарыйИД, 1000, Истина);
...
Процедура ПриНажатииКнопки(Кнопка)
	Если Не ПроверитьРазрешениеИР() Тогда
		Возврат;
	КонецЕсли;
	Если Кнопка = "РедактироватьТекст" Тогда
		РедактироватьТекст();
	ИначеЕсли Кнопка = "ОтладитьОбъект" Тогда
		ОтладитьОбъект();
	ИначеЕсли Кнопка = "ФильтрИзСинонима" Тогда
		УстановитьФильтрТипаИзСинонимаРеквизита();
	ИначеЕсли Кнопка = "СписокОбщихМетодов" Тогда
		СписокОбщихМетодов();
	ИначеЕсли Кнопка = "ВыполнитьПроизвольныйКод" Тогда
		ВыполнитьПроизвольныйКод();
	ИначеЕсли Кнопка = "ВставитьУсловниеИзТабло" Тогда
...
Процедура ВыполнитьПроизвольныйКод()
	...
	ТурбоКонф.НачатьВызовВнешнегоОкнаАсинх("^Текст.*", 5000, ОкноВыражения, РежимОткрытияВнешнегоОкна.Диалог, Истина, ИдентификаторПроцессаИР);
	СтруктураВызова = ПолеТекстаПрограммы.ОткрытьРедакторКодаДляОтладчика(ТекстПоля, ТекстОшибки);
	ОповеститьПользователяОВозобновлении();
	ЗавершитьВызовВнешнегоОкнаАсинх(ОкноВыражения, ИдентификаторПроцессаИР);
	ПодключениеИР.Visible = Ложь;
	...
	ТурбоКонф.КонтролКлавиша(Клавиши.Enter);
	ТурбоКонф.Клавиша(Клавиши.Escape);
	ТурбоКонф.ЖдатьЗакрытияОкна(ОкноВыражения,, 1000,, Ложь);
	УстановитьФокусВПолеВводаВыраженияОкнаВыражение();
...
Процедура Тест()
	ТурбоКонф.ПерейтиВПозицию(0, 3321);
