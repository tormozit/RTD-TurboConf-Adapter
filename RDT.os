...
Процедура АктивироватьСтрокуДерева(Знач Путь) Экспорт
	Для Каждого Фрагмент Из Путь Цикл
		ТурбоКонф.КликнутьПоЭлементу(Фрагмент, "элемент списка", 0, Ложь);
...
Функция НайтиУзелДерева(Знач ПодключениеИР, Знач ТекстИскомогоЭлемента, Знач СвернутьДерево = Ложь) Экспорт
	...
	ТурбоКонф.ВставитьТекст(ТекстИскомогоЭлемента);
	ТурбоКонф.Клавиша(Клавиши.Enter);
	ТурбоКонф.Ждать(100);
	ИмяТекущегоЭлемента = ТурбоКонф.ПолучитьИмяЭлемента();
	Сообщить("ИмяТекущегоЭлемента=" + ИмяТекущегоЭлемента);
...
Функция НаборСловИзИР(ПолеТекстаПрограммы, ИмяНабораСлов = "")
	...
	Для Каждого СтрокаСлова Из ТаблицаСлов Цикл
		...
		Иначе
			...
			//	ТекстШаблона = СтрокаСлова.Слово + "(""ru='<?>'"")"; // Лишняя пара скобок вставляется https://turboconf.ru/Tasks/9522
			КонецЕсли;
		КонецЕсли;
