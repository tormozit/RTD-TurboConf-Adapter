...
// Окно "Выражение"
//@button {'Name': 'КнопкаОтладитьОбъект', 'Text': 'Отладить', Hint:'Отладить объект в подходящем инструменте (ИР)', 'Image': '', 'Anchor': 'RightTop', 'Offset': '-190,50', 'Width': 60, 'Height': 18, 'BackColor': '', 'Filter': '^Выражение$' }
//@button {'Name': 'КнопкаРедактироватьТекст', 'Text': 'Редактор', Hint:'Редактировать многострочный текст в редакторе текста (ИР)', 'Image': '', 'Anchor': 'RightTop', 'Offset': '-255,50', 'Width': 90, 'Height': 18, 'BackColor': '', 'Filter': '^Выражение' }
//@button {'Name': 'КнопкаВыполнитьПроизвольныйКод', 'Text': 'Выполнить', Hint:'Открыть редактор кода и после нажатия в нем ОК выполнить введенный код (ИР)', 'Image': '', 'Anchor': 'RightTop', 'Offset': '-350,50', 'Width': 70, 'Height': 18, 'BackColor': '', 'Filter': '^Выражение$' }
//@button {'Name': 'КнопкаПрерватьОтлаживаемыйКод', 'Text': 'Прервать', Hint:'Будут очищены все локальные переменные и выполнена команда "Шагнуть из". Этого обычно достаточно для вызова ошибки выполнения в отлаживаемом коде. (ИР)', 'Image': '', 'Anchor': 'RightTop', 'Offset': '-425,50', 'Width': 60, 'Height': 18, 'BackColor': '', 'Filter': '^Выражение$' }
...
//@button {'Name': 'КнопкаВставитьОшибкуИзБуфера', 'Text': 'Вставить (ИР)', Hint:'Вставить текст ошибки из буфера обмена и окна сообщений и включить остановку (ИР)', 'Image': '', 'Anchor': 'LeftTop', 'Offset': '170,60', 'Width': 110, 'Height': 18, 'BackColor': '', 'Filter': '^Остановка по ошибке$' }
...
//@button {'Name': 'КнопкаВставитьИзРезультатовПоиска', 'Text': 'Из результатов поиска', Hint:'Вставить точки останова из уже выделенных в результатах поиска строк (ИР)', 'Image': '', 'Anchor': 'RightTop', 'Offset': '-290,10', 'Width': 130, 'Height': 18, 'BackColor': '', 'Filter': '^Точки останова$' }
// Окно "Закладки"
...
//@button {'Name': 'КнопкаУстановитьТипПараметраВыбора', 'Text': 'Установить тип (ИР)', Hint:'Установить тип параметру выбора (ИР)', 'Image': '', 'Anchor':'LeftTop', 'Offset': '110,40', 'Width': 110, 'Height': 18, 'BackColor': '', 'Filter': '^Редактирование параметров выбора$' }
...
// Окно "Событие"
//@button {'Name': 'ПерейтиКСтрокеМодуляИзСобытия', 'Text': 'Перейти (ИР)', Hint:'Перейти в модуль из описания ошибки (ИР)', 'Image': '', 'Anchor': 'LeftTop', 'Offset': '15,125', 'Width': 80, 'Height': 18, 'BackColor': '', 'Filter': '^Событие' }
// Окно "Пользователь"
//@button {'Name': 'РедактироватьПользователя', 'Text': 'Изменить (ИР)', Hint:'Редактировать пользователя в расширенной форме (ИР)', 'Image': '', 'Anchor': 'LeftBottom', 'Offset': '20,-34', 'Width': 80, 'Height': 20, 'BackColor': '', 'Filter': '^Пользователь' }
...
Процедура УстановитьФильтрВыбораТипа() Экспорт
	...
	Если ПодключениеИР = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ОкноРедактораТипа = ТурбоКонф.ПолучитьАктивноеОкно();
	ТурбоКонф.Клавиша(Клавиши.Escape); 
	ТурбоКонф.ЖдатьЗакрытияОкна(ОкноРедактораТипа,, 1000);
	ТурбоКонф.Ждать(50);
...
Процедура ВыбратьТолькоПомеченныеТипы() Экспорт
	...
	Если Индекс = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ТурбоКонф.УстановитьФокусНаЭлемент("", "список");
...
Процедура УстановитьТипПараметраВыбора() Экспорт
	...
	ТурбоКонф.УстановитьФокусНаЭлемент("", "таблицу");
...
Процедура АктивироватьСвойствоВПалитреСвойств(Знач ИмяСвойства) Экспорт
...
Процедура АктивироватьСтрокуДерева(Знач Путь) Экспорт
...
Процедура ПерейтиКОпределению()
	...
	Если ПродолжатьОбработку Тогда
		...
		ИначеЕсли Истина
			...
			И Значение = "*"
		Тогда
			// Чтобы открылась панель свойств текущего объекта метаданных из списка
...
//
Процедура ПерейтиВПозицию(Знач НомерКолонки = 0, Знач НомерСтроки = 0, Знач ПолеТекстаПрограммы = Неопределено, Знач Строгое = Ложь)
...
//&ТолькоТекстовыеДокументы
Процедура ПерейтиВНачалоРодителя()
	ПодключениеИР = ПодключениеИР(,, Ложь);
	Если ПодключениеИР = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Если ЭтоПолноеИмяМД(МодулиИР, СсылкаСтроки) Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(ИмяЭлементаМакета) Тогда
			...
			Если ТурбоКонф.ПолучитьЗаголовокТекущегоОкна() = "Имена" Тогда
				//ТурбоКонф.УстановитьЗначение(ИмяЭлементаМакета); // не срабатывает обработчик изменения
				ТурбоКонф.ВставитьТекст(ИмяЭлементаМакета);
...
Процедура НайтиСтрокуВОкне(Знач ИскомаяСтрока, Знач РазвернутьВсеУзлыДерева = Ложь, Знач ЖдатьЧерезЗаголовокОкна = Истина) Экспорт
	Если РазвернутьВсеУзлыДерева Тогда
...
Процедура ВставитьИзРезультатовПоиска()
	ТурбоКонф.ШифтКлавиша(Клавиши.Escape);
...
Процедура ВычислитьВыражение()
	...
	Если Не СтрНачинаетсяС(ТекстПоля, ТекстВызова) Тогда
		Выражение = ТекстВызова + "(Новый Структура(""_"", " + ТекстПоля + "))";
...
Функция НайтиВСписке(Знач ЯчейкиТекущейСтроки = Неопределено)
	...
	Если Найти(Заголовок, "- Конфигуратор - ") > 0 Тогда
	...
	Если ЛиТаблица И ЧислоКолонок = Неопределено Тогда
		МаксЧислоКолонок = 25;
	...
	КонецЕсли;
	ИндексВыбраннойСтроки = ВыбратьЭлементСписка(ТекстыЯчеек, ГрязныйТекстТекущейЯчейки,, ЛиТаблица, Заголовок, ЧислоКолонок, НомерСтрокиСписка);
	Если ИндексВыбраннойСтроки = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	Если Ложь
		Или Не ПоддерживаетПоиск
		...
		Или (Истина
			...
			И ЧислоКолонок > 1 
			И ИндексВыбраннойСтроки < МаксНомерСтрокиДляТочногоПозиционирования())
	Тогда
		ТурбоКонф.Клавиша(Клавиши.Home);
		НажатьКлавишуЦикл(Клавиши.Down, ИндексВыбраннойСтроки);
	Иначе
		ГрязныйТекстТекущейЯчейки = ТекстыЯчеек[ЧислоКолонок*ИндексВыбраннойСтроки + ИндексКлючевойКолонки];
...
Функция ЯчейкиТекущейСтрокиСписка(Знач Иерархический = Ложь) Экспорт
	...
	СтарыйИД = Неопределено;
	Для НомерКолонки = 1 По ?(Иерархический, 1, 10) Цикл
		...
		Если ТекстЯчейки = "" И НомерКолонки = 1 Тогда
			// Точки останова - странная колонка в крайней правой позиции
		Иначе
...
Функция ВыбратьЭлементСписка(Знач ТекстыЯчеек, Знач ТекстТекущейЯчейки = "", Знач ЛиМодально = Истина, Знач ЛиТаблица = Ложь, Знач Заголовок = Неопределено, Знач ЧислоКолонок = Неопределено,
		Знач НомерСтрокиСписка = Неопределено) Экспорт
	ПодключениеИР = ПодключениеИР();
	Если ПодключениеИР = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	КонецЕсли;
	Попытка
		ИндексВыбранногоЭлемента = МодулиИР.ирКлиент.ОткрытьТаблицуИзМассиваИзСтрокиВнутрЛкс(ТекстыПолейСтрока, ТекстТекущейЯчейки, ЛиТаблица, ЛиМодально, ЧислоКолонок, Заголовок, КлючХраненияВыбора, НомерСтрокиСписка);
	Исключение
		Если ЛиМодально Тогда
			ОписаниеОшибки = ОписаниеОшибки();
			ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), ОписаниеОшибки);
	...
	КонецЕсли;
	Возврат ИндексВыбранногоЭлемента;
КонецФункции
...
Процедура ОтладитьОбъект()
	...
	Если Не Успех Тогда
		...
		ШаблонЗаголовкаОкна = "Консоль запросов|Исследователь объектов";
...
Функция ПолеТекстаПрограммыИР(Знач ПодключениеИР, Знач ТекстПоля = "", Знач Позиция0Поле = 0, Знач ВыделенныйТекст = "", ПереходитьВоВложенныйКонтекст = Истина, Знач КакВызовМетода = Неопределено,
	...
	ИначеЕсли РазрешитьПоследнийДокумент Тогда
		...
		КонецПопытки;
		Если ПоследнийДокумент.Заголовок = "Ввод выражения" Тогда
...
Процедура ПерейтиВПоследниеМетоды()
	...
	Если ПолеТекстаПрограммы <> Неопределено Тогда
		...
		КонецЕсли;
	КонецЕсли;
	ПерейтиПоСсылкеСтрокиМодуля(Ссылка);
КонецПроцедуры
...
Процедура РедактироватьКомпоновку()
	...
	Для Счетчик = 1 По 7 Цикл
		...
		ТурбоКонф.КонтролКлавиша(Клавиши.PageUp);
	КонецЦикла;
	КликнутьПоЭлементу(Заголовок, "кнопка");
	...
	Пока Истина Цикл
		...
		Иначе
			...
			Если ОпределениеИзВерсии <> ОпределениеРабочее Тогда
				...
				Возврат;
			КонецЕсли;
			// Закрываем окно сравнения текстов модулей
...
Процедура НайтиПоследнююПравкуВГите()
	...
	ТурбоКонф.ЖдатьСменуТекущегоЭлемента(СтарыйИД, 1000, Истина);
	//ТурбоКонф.ВставитьТекст(XMLСтрока(НомерСтроки)); // Делает двойную вставку // часто вызывает ошибку "Сбой при выполнении запрошенной операции с буфером обмена"
	//ТурбоКонф.КонтролКлавиша(Клавиши.Z); // Откатывает вторую вставку
	ТурбоКонф.ОтправитьТекст(XMLСтрока(НомерСтроки)); // Работает
	//ТурбоКонф.УстановитьЗначение(XMLСтрока(НомерСтроки)); // Не работает
	ТурбоКонф.Клавиша(Клавиши.Enter);
КонецПроцедуры

Процедура НажатьКлавишуЦикл(Знач Клавиша, Знач Количество, Знач Задержка = 0)
	Для Счетчик = 1 По Количество Цикл
		ТурбоКонф.Клавиша(Клавиша);
		ТурбоКонф.Ждать(Задержка);
	КонецЦикла;
КонецПроцедуры
...
Процедура СообщитьОПроблеме()
	...
	ТехническаяИнформация = 
		...
		|Адаптер. Версия: " + ВерсияАдаптера;
	Если ПолеТекстаПрограммы <> Неопределено Тогда
		ТехническаяИнформация = ТехническаяИнформация + "
			|Конфигуратор. ИмяМодуля: " + ПолеТекстаПрограммы.мИмяМодуля + "
			|Конфигуратор. ПозицияВМодуле: " + ПолеТекстаПрограммы.мНачальнаяСтрока + ", " + ПолеТекстаПрограммы.мНачальнаяКолонка;
	КонецЕсли;
	ТехническаяИнформация = ТехническаяИнформация + "
		|-----------------------------
...
Процедура ПриНажатииКнопки(Кнопка)
	...
	ИначеЕсли Кнопка = "КнопкаНайтиКартинку" Тогда
		КнопкаНайтиКартинку();
	ИначеЕсли Кнопка = "КнопкаВставитьИзРезультатовПоиска" Тогда
		ВставитьИзРезультатовПоиска();
...
Процедура ПерейтиКСтрокеМодуляИзСобытия()
	ТурбоКонф.УстановитьФокусНаЭлемент("Справка", "кнопка");
...
Процедура ВыполнитьПроизвольныйКод()
	...
	КонецЕсли;
	ТурбоКонф.ШифтКлавиша(Клавиши.Tab);
	ТурбоКонф.ВставитьТекст(ВременноеВыражение);
	ТурбоКонф.Ждать(100); // 50 - недостаточно
	ТурбоКонф.Клавиша(Клавиши.Enter);
	ТурбоКонф.ШифтКлавиша(Клавиши.Tab);
	ТурбоКонф.Клавиша(Клавиши.Add);
	ТурбоКонф.Клавиша(Клавиши.Down);
	...
	Окно = ТурбоКонф.ПолучитьАктивноеОкно();
	ИдентификаторПроцессаИР = МодулиИР.ирКэш.ИдентификаторПроцессаОСЛкс(); 
	//ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), "Запущена передача контекста в редактор кода ИР"); // Не исчезает до закрытия окна
	ОповеститьПользователяОПриостановке(ПодключениеИР);
	ТурбоКонф.НачатьВызовВнешнегоОкнаАсинх("^Текст.*", 5000, Окно, РежимОткрытияВнешнегоОкна.Диалог, Истина, ИдентификаторПроцессаИР);
	Попытка
		СтруктураВызова = ПолеТекстаПрограммы.ОткрытьРедакторКодаДляОтладчика(ТекстПоля, ТекстОшибки);
	Исключение
		ОписаниеОшибки = ОписаниеОшибки();
		ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), ОписаниеОшибки);
		Сообщить(ОписаниеОшибки, СтатусСообщения.Важное);
	...
	УстановитьФокусВПолеВводаВыраженияОкнаВыражение();
	ТурбоКонф.ВставитьТекст("ирОбщий.Оперировать(ирОбщий.ВосстановитьЗначениеЛкс(""" + ИмяХранилища + """), 2, " + """" + СтруктураВызова.ИменаПараметров + """, " + СтруктураВызова.ИменаПараметров + ")");
	ТурбоКонф.Ждать(50); 
	ТурбоКонф.Клавиша(Клавиши.Enter);
КонецПроцедуры
...
//&ВосстанавливатьБуферОбмена
Процедура ВставитьУсловниеИзТабло()
	ПодключениеИР = ПодключениеИР();
	Если ПодключениеИР = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ЧислоКолонокВСтроке = 3;
	Окно = ТурбоКонф.ПолучитьАктивноеОкно();
	ТурбоКонф.Клавиша(Клавиши.Escape);
	ТурбоКонф.ЖдатьЗакрытияОкна(Окно,, 1000,, Ложь);
	...
	ТурбоКонф.КопироватьТекстВБуфер();
	ТурбоКонф.Ждать(100);
	ТекстыЯчеек = СтрРазделить(ТурбоКонф.ПолучитьБуферОбмена(), Символы.Таб + Символы.ПС);
	ЭлементыУсловия = Новый Массив;
	Для НомерСтроки = 1 По ТекстыЯчеек.Количество() / ЧислоКолонокВСтроке Цикл
		ИндексПервойЯчейки = (НомерСтроки-1)*ЧислоКолонокВСтроке;
		ТекстВыражения = ТекстыЯчеек[ИндексПервойЯчейки + 0];
		ЗначениеВыражения = ТекстыЯчеек[ИндексПервойЯчейки + 1];
		Если Лев(ЗначениеВыражения, 1) = """" Тогда
...
Процедура ВставитьОшибкуИзБуфера()
	...
	Для Каждого Причина Из ПричиныОшибок Цикл
		...
		ТурбоКонф.ВставитьТекст(Причина);
	КонецЦикла;
	ТурбоКонф.Клавиша(Клавиши.Enter);
КонецПроцедуры
