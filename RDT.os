...
//@script_hotkey Control+Alt+F12 НайтиВМодулях ИР Найти в модулях
...
//@script_hotkey None            ОткрытьСписок ИР Открыть список в ИР
...
Процедура ПерейтиКОпределению()
	...
	Если ПродолжатьОбработку Тогда
		...
		Если Значение = Истина Тогда
			...
			ВыделениеДвумерное = ПолеТекстаПрограммы.ПолеТекста.ВыделениеДвумерное();
			ТурбоКонф.ПерейтиВПозицию(ВыделениеДвумерное.НачальнаяКолонка, ВыделениеДвумерное.НачальнаяСтрока); // В API 6+ нужно только для запоминания позиции в истории переходов конфигуратора
			Если ТурбоКонф.ПолучитьВерсиюAPI() >= 6 Тогда
				ВыделениеОдномерное = ПолеТекстаПрограммы.ПолеТекста.ВыделениеОдномерное();
				ТурбоКонф.ВыделитьТекст(ВыделениеОдномерное.Начало - 1, ВыделениеОдномерное.Конец - 1);
			КонецЕсли;
...
Функция ПерейтиПоСсылкеСтрокиМодуля(Знач СсылкаСтроки = "", Знач ИмяРасширения = "")
	...
	Иначе
		...
		Если ЗначениеЗаполнено(СтруктураСсылки.Метод) Тогда
			ЖдатьПотерюАктивностиОкна(СтароеОкно);
			...
			Тогда
				...
				ТурбоКонф.ПерейтиВПозицию(ПолеТекстаПрограммы.мНачальнаяКолонка, ПолеТекстаПрограммы.мНачальнаяСтрока); // Так всегда будет в центре по высоте и запомнится позиция в истории переходов
				Позиция = ПолеТекстаПрограммы.ПолеТекста.ПозицияВПолеТекстаПоНомеруСтрокиИКолонки(ПолеТекстаПрограммы.мНачальнаяСтрока, ПолеТекстаПрограммы.мНачальнаяКолонка) - 1;
				РазвернутьБлокМодуля(Позиция, Позиция);
	...
	Если ДокументыМодулей.Количество() > 0 Тогда
		...
		Для Индекс = 1 - ДокументыМодулей.Количество() По 0 Цикл // Обратный обход
			...
			Тогда
				...
				Продолжить;
			КонецЕсли;
			Если Не ЗначениеЗаполнено(Документ.ИмяКонфигурации) Тогда
...
Процедура ПоказатьПараметрыМетода()
	...
	ПодключениеИР.Visible = Истина;
	ОповеститьПользователяОПриостановке();
	ТурбоКонф.НачатьВызовВнешнегоОкнаАсинх("^Метод.*", 3000, Конфигуратор.ГлавноеОкно, РежимОткрытияВнешнегоОкна.Диалог, Истина, МодулиИР.ирКэш.ИдентификаторПроцессаОСЛкс(), ПозицияКаретки);
	Попытка
...
Функция МодулиИР(Знач Приложение, Знач ЭтоПервоеПодключение = Ложь) Экспорт
	Попытка
		...
		//ирПлатформа = Приложение.ирПлатформа; // Вызывает 200мс задержку в некоторых случаях
		Значение = Приложение;
	Исключение
		...
		КонецЕсли;
	КонецПопытки; 
	Если ирПлатформа = "ВерсияИРОбновлена" Тогда
...
Процедура ОбработчикОтветаЗаполнитьКэшМодулей(Значение, Параметры) Экспорт
	...
	ИДКонфигуратора = Конфигуратор.PID;
	ТурбоКонф.АктивироватьОкно(Конфигуратор.ГлавноеОкно);
	СтрокаКоманды = ЗапуститьТолстыйКлиент(Ложь, Ложь, "/CОбновитьКэшМодулейИР;" + (Не Булево("" + Значение)), ИмяИсполняемогоФайла, Значение = КодВозвратаДиалога.Да);
...
Функция ЗапуститьТолстыйКлиент(Знач ПодключитьОтладчик = Истина, Знач СПользователем = Истина, Знач ДопПараметры = "", Знач ИмяИсполняемогоФайла = "", Знач ТолькоВернутьСтроку = Ложь)
	...
	Если ПодключитьОтладчик Тогда
		...
		КонецЕсли;
	КонецЕсли;
	Возврат ЗапуститьПриложение1С("/DEBUG" + ДобавкаАдресОтладчика + " " + ДобавкаПортативныйИР + " " + ДопПараметры, СПользователем, ИмяИсполняемогоФайла, ТолькоВернутьСтроку);
КонецФункции

// ИмяИсполняемогоФайла передаем когда нет активного конфигуратора
Функция ЗапуститьПриложение1С(Знач ДопПараметры, Знач СПользователем = Истина, Знач ИмяИсполняемогоФайла = "", Знач ТолькоВернутьСтроку = Ложь)
