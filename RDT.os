...
//@script_hotkey Shift+Alt+F9    ВычислитьВыражение ИР Вычислить выражение
//@script_hotkey Control+Shift+Q ЗакрытьПриложениеИР ИР Закрыть приложение ИР
//@script_hotkey Control+F11     КопироватьСсылку ИР Копировать ссылку
//@script_hotkey Control+Alt+F12 НайтиСсылкиНаМетод ИР Найти ссылки на метод
//@script_hotkey Alt+W           ОткрытьСписок ИР Открыть список в ИР
//@script_hotkey Alt+Q           ОткрытьОбъект ИР Открыть объект в ИР
...
//@script_hotkey Alt+F3          ПоказатьПараметрыМетода ИР Показать параметры метода
//@script_hotkey Alt+F7          ПроверитьМодуль ИР Проверить модуль
...
//@script_hotkey Control+Alt+F9  ТочкаОстанова ИР Точка останова
//@script_hotkey Alt+Shift+F     ФорматироватьТекст ИР Форматировать текст
//script_hotkey None             АвтодополнениеМодальное ИР Автодополнение модальное
//@script_hotkey None            АнализЗамера ИР Анализ замера
...
//@script_hotkey None            ПереименоватьСлово ИР Переименовать слово
//@script_hotkey None            ПодпискиНаСобытия ИР Подписки на события
//@script_hotkey None            РедактироватьКомпоновку ИР Редактировать компоновку
//@script_hotkey None            СброситьНастройкиИРБазы ИР Сбросить настройки ИР базы
...
//@script_hotkey None            СохранитьМодульВФайл ИР Сохранить модуль в файл
//@retain_clipboard 0
//@enterprise_mode 0
//@hide_actions 0
//@min_api_version 5
...
Процедура ПередатьИзмененияИзПоляТекстаВОкноМодуля(Знач ПолеТекстаПрограммы, Знач ВесьМодуль = Ложь) Экспорт
	Пустышка = 0;
	...
	Иначе
		Если ЯзыкПрограммы = 0 И Не ВесьМодуль И ПолеТекстаПрограммы.мМетодМодуля <> Неопределено Тогда
...
Процедура ФорматироватьТекст()
	...
	КонецЕсли;
	ВыделениеДвумерное = ПолеТекстаПрограммы.ПолеТекста.ВыделениеДвумерное();
	ПолеТекстаПрограммы.СлужебноеПолеТекстаДолгое.УстановитьГраницыВыделения(1,1, ВыделениеДвумерное.НачальнаяСтрока, 1);
	ПозицияНачала = СтрДлина(ПолеТекстаПрограммы.СлужебноеПолеТекстаДолгое.ВыделенныйТекст);
	КонечнаяСтрока = ВыделениеДвумерное.КонечнаяСтрока;
	Если ВыделениеДвумерное.КонечнаяКолонка > 1 Тогда
		КонечнаяСтрока = КонечнаяСтрока + 1;
	КонецЕсли;
	ПолеТекстаПрограммы.СлужебноеПолеТекстаДолгое.УстановитьГраницыВыделения(1,1, КонечнаяСтрока, 1);
	ПозицияКонца = СтрДлина(ПолеТекстаПрограммы.СлужебноеПолеТекстаДолгое.ВыделенныйТекст);
	ПолеТекстаПрограммы.ФорматироватьТекстВстроенногоЯзыка(Ложь);
	Пустышка = 0;
	НовыйТекстМодуляИзКонфигуратора = ТурбоКонф.ПолучитьТекстМодуля(ВыделенныйТекст, Пустышка);
	Если ТурбоКонф.ПолучитьВерсиюAPI() < 6 Тогда
		Если Истина
			...
			И ПолеТекстаПрограммы.мКонечнаяСтрока <= ПолеТекстаПрограммы.мНомерПоследнейСтрокиТелаМетода + 1 
		Тогда
			// Заменяем только текущий метод
			ПолеТекстаПрограммы.РазобратьТекущийКонтекст();
			ТекстВставки = ПолеТекстаПрограммы.ОпределениеМетодаБезОписания();
		...
		КонецЕсли;
	Иначе 
		ПолеТекстаПрограммы.УстановитьГраницыВыделения(ВыделениеДвумерное.НачальнаяСтрока, 1, КонечнаяСтрока, 1);
		ТекстВставки = ПолеТекстаПрограммы.ВыделенныйТекст();
		ТурбоКонф.ВыделитьТекст(ПозицияНачала, ПозицияКонца);
	КонецЕсли;
	ТурбоКонф.ВставитьТекст(ТекстВставки);
	НомерСтроки = ВыделениеДвумерное.НачальнаяСтрока;
	НомерКолонки = ВыделениеДвумерное.НачальнаяКолонка;
	//ТурбоКонф.ПерейтиВПозицию(НомерКолонки, НомерСтроки); // слишком долго
...
Функция ЖдатьПотерюАктивностиОкна(Знач СтароеАктивноеОкно = Неопределено, Знач ЧислоСекунд = 2) Экспорт
	...
	Цикл
		ТурбоКонф.Ждать(100);
	КонецЦикла;
	Возврат ТурбоКонф.ПолучитьАктивноеОкно() <> СтароеАктивноеОкно;
КонецФункции
...
// Возвращаемое значение:
//    Строка - 
Функция СсылкаНаОбъектМДВДеревеКонфигурации(Знач ПодключениеИР, Знач ТолькоВерхнийУровень = Ложь) Экспорт
	МодулиИР = МодулиИР(ПодключениеИР);
...
Процедура ПерейтиКОпределению()
	...
	Если НомерСтроки > 0 Тогда
		...
		Иначе
			...
			Если Значение = Неопределено Тогда
				Если Истина     
					И МодулиИР.ирОбщий.СтруктураСсылкиСтрокиМодуляЛкс(ЗначениеИзБуфера) <> Неопределено
	...
	Если ПолеТекстаПрограммы = Неопределено И РазрешитьСсылкуИзТекущейПозиции Тогда 
		...
		Если Значение.Количество() = 0 Тогда
			Значение = ТекстМодуля;
			ТекстМодуля = "//" + СтрРазделить(ТекстМодуля, " ")[0]; // Отделять пробелом нужно для ячеек табличных документов. В их имя в конце добавляется адрес через пробел, например "<ТекстЯчейки> A3" 
	...
	Если ТипЗнч(Значение) = Тип("COMОбъект") Тогда
		...
		Иначе
			...
			Иначе
				...
				Если ПолеТекстаПрограммы <> Неопределено Тогда 
					РезультатПереходаИР = ПолеТекстаПрограммы.ПерейтиПоСсылкеСтрокиМодуля(НоваяЦельПерехода.Значение);
					ПродолжатьОбработку = РезультатПереходаИР <> Неопределено;
				КонецЕсли;
	...
	Если ПродолжатьОбработку Тогда
		...
		Если Значение = Истина Тогда
			...
			//	НачальнаяПозиция=0;
			//	КонечнаяПозиция=0;
			//	// Выходные параметры ОСкрипт 1.7 не поддерживает
			//	ПолеТекстаПрограммы.ПолеТекста.ВыделениеОдномерное(НачальнаяПозиция, КонечнаяПозиция);
			//	ТурбоКонф.ВыделитьТекст(НачальнаяПозиция, КонечнаяПозиция);
			//Иначе 
				ВыделениеДвумерное = ПолеТекстаПрограммы.ПолеТекста.ВыделениеДвумерное();
...
// Портит буфер обмена
Функция ПерейтиПоСсылкеСтрокиМодуля(Знач Значение = "", Знач ИмяРасширения = "")
	// СтароеЗначениеБуфераОбмена = ТурбоКонф.ПолучитьБуферОбмена();
	ПодключениеИР = ПодключениеИР(,, Ложь);
	...
	Если ЭтоПолноеИмяМД(МодулиИР, Значение) Тогда
		Если ЗначениеЗаполнено(ИмяРасширения) Тогда
			Значение = ИмяРасширения + " " + Значение;
		КонецЕсли;
		Попытка
			ТурбоКонф.Перейти(Значение);
			Успех = Истина;
		Исключение
			Сообщить("Ошибка перехода к объекту метаданных """ + Значение + """: " + ОписаниеОшибки(), СтатусСообщения.Внимание);
			Успех = Ложь;
		КонецПопытки;
	...
	Пока ТекущаяДата() - НачМомент < 2 Цикл
		...
		Пока Истина Цикл
			...
			Иначе
				...
				//
				//Попытка
				//	ТекстыПолей = ТурбоКонф.ПолучитьТекстТаблицы(); // Долго, т.к. через буфер обмена копирует строки
				//Исключение
				//	// TODO Ошибка если в таблице одна строка https://turboconf.ru/Tasks/8979
				//	// Опасно, т.к. не проверяем что это наша ссылка и считаем что это она
				//	ПредСтрока = "Куку";
				//КонецПопытки;
				//ТекстыПолей = СтрРазделить(ТекстыПолей, Символы.ПС + Символы.Таб); 
			...
			Если ТекстыПолей.Количество() >= 3 Тогда
				...
				ТекстПоследнейСтроки = "";
				Для Счетчик = -ЧислоКолонок По -1 Цикл
					ТекстПоследнейСтроки = ТекстПоследнейСтроки + ТекстыПолей[ТекстыПолей.Количество() + Счетчик];
				КонецЦикла;
			Иначе
			...
			Если ТурбоКонф.КликнутьПоЭлементу(ИмяЯчейки, Неопределено, 0, Ложь) Тогда // Было 1000мс. Исправлено
		...
		Если ЗначениеЗаполнено(СтруктураСсылки.Метод) Тогда
			...
			Тогда
				...
				Если ОписаниеМетода <> Неопределено Тогда
					...
					Если Истина
						И Ложь // Пока отключил из-за неудобного позиционирования https://turboconf.ru/Tasks/8888
						И ТурбоКонф.ПолучитьВерсиюAPI() >= 6 
					Тогда
						ТурбоКонф.ВыделитьТекст(ОписаниеМетода.ПозицияОпределения, ОписаниеМетода.ПозицияОпределения);
						РазвернутьБлокМодуля(); // Сбрасывает выделение
						ТурбоКонф.ВыделитьТекст(ОписаниеМетода.ПозицияОпределения, ОписаниеМетода.ПозицияОпределения + 1);
					Иначе 
...
Функция ЗагрузитьТочкиОстановаИзФайлаВОткрытомОкнеТочекОстанова(ИмяФайла, СтарыйИД = Неопределено)
...
Функция ПолучитьВыделенныйТекст(ТолькоЧерезБуферОбмена = Истина)
	// Еще есть ТурбоКонф.ПолучитьТекстТекущегоЭлемента()
	Текст = Неопределено;
	Если Не ТолькоЧерезБуферОбмена Тогда 
		Если Ложь 
	...
	КонецЕсли;
	Сообщить("Текст элемента интерфейса = " + Лев(Текст, 1000));
	Возврат Текст;
КонецФункции
...
Процедура ПриПолученииДанныхТ9(Параметры)
	...
	Если Найти(ТипКонтекста, "+") > 0 Тогда
		Попытка
			ИменаТипов = ПолеТекстаПрограммы.ПредставлениеМассиваСтруктурТипов(ПолеТекстаПрограммы.мТаблицаТиповКонтекста);
		Исключение
			// ИР 7.00-
			ИменаТипов = "";
		КонецПопытки;
...
//  ПодключениеИР					 - 	 - 
//  ТекстМодуля						 - 	 - 
//  ПозицияВТексте					 - Число - если 0, то текущий документ получается заново
//  ВыделенныйТекст					 - 	 - 
...
//   - 
//
Функция ПолеТекстаПрограммыИР(Знач ПодключениеИР, Знач ТекстПоля, Знач ПозицияВПоле, Знач ВыделенныйТекст = "", ПереходитьВоВложенныйКонтекст = Истина, Знач КакВызовМетода = Неопределено,
	...
	Тогда
		...
		Сообщить("текст поля = " + Лев(ТекстПоля, 1000));
		...
		ПолеТекстаПрограммы.ПрочитатьНачалоИКонецТекущейСтроки(ТекстПоля, ПозицияВПоле + 1);
	...
	КонецЕсли;
	Сообщить("Синхронизация текущего модуля " + (ТекущаяУниверсальнаяДатаВМиллисекундах() - МоментЗапуска) + " мс");
...
Процедура ПередатьИзмененияМодулей(Знач ПодключениеИР, Знач ПолеТекстаПрограммы = Неопределено) Экспорт
	...
	Если ДокументыМодулей.Количество() > 0 Тогда
		...
		КонецЕсли;
		Для Каждого Документ Из ДокументыМодулей Цикл
			Если Истина
			...
			КонецЕсли;
			МоментЗапуска = ТекущаяУниверсальнаяДатаВМиллисекундах();
			ПолноеИмяМодуля = ИмяМодуляДокумента(ПодключениеИР, Документ.Текст, Документ.ИмяКонфигурации, Документ.Заголовок, Ложь);
...
Процедура ПереименоватьСлово()
	...
	ОповеститьПользователяОПриостановке();
	ТурбоКонф.НачатьВызовВнешнегоОкнаАсинх("^Переименовать.*", 2000, Конфигуратор.ГлавноеОкно, РежимОткрытияВнешнегоОкна.Диалог, Истина, МодулиИР.ирКэш.ИдентификаторПроцессаОСЛкс(), ПозицияКаретки);
	Попытка
...
Процедура АнализЗамера()
	...
	УстановитьИмяФайлаВДиалогеСохранения(ИмяФайла, "Сохранение");
	ТурбоКонф.Клавиша(Клавиши.Enter);
	МодулиИР = МодулиИР(ПодключениеИР);
	ВосстановитьОкноПриложения(ПодключениеИР);
	ТурбоКонф.Ждать(100);
...
// Параметры:
//    ИмяФайла - Строка - 
Процедура УстановитьИмяФайлаВДиалогеСохранения(ИмяФайла, ЗаголовокОкна) Экспорт
...
Процедура РедактироватьКомпоновку()
	...
	УстановитьИмяФайлаВДиалогеСохранения(ИмяФайла, Заголовок);
	ТурбоКонф.Клавиша(Клавиши.Enter);
	ТурбоКонф.Ждать(100);
	МодулиИР = МодулиИР(ПодключениеИР);
	ВосстановитьОкноПриложения(ПодключениеИР);
...
Функция ПодключениеИР(ПарольИлиСтрокаСоединения = Неопределено, РазобратьПозициюВТексте = Истина, Знач ВидимостьДляНовогоПодключения = Ложь, Знач ЭтоДокумент = Неопределено, выхПодключеноНоНетПодсистемы = Ложь) Экспорт
	...
	Если ЭтоДокумент = Неопределено Тогда
		ТипЭлементаИнтерфейса = ПолучитьТипТекущегоЭлемента(); // 9/20мс
	...
	КонецЕсли;
	УведомлениеЗапуска = ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), "Запускается приложение " + НазваниеПриложения() + ". Закрыть его можно командой ИР.ЗакрытьПриложениеИР.", 30000);
	Попытка
		...
		Если Не УспехПодключения Тогда
			...
			Иначе
				...
				Попытка
					УспехПодключения = ПодключениеИР.Connect(СтрокаСоединения);
				Исключение
					ОписаниеОшибки = "Ошибка подключения приложения ИР через COM интерфейс V83.Application:
					|"+ ОписаниеОшибки();
					ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), ОписаниеОшибки);
					Сообщить(ОписаниеОшибки, СтатусСообщения.Важное);
					УспехПодключения = Ложь;
...
Функция МодулиИР(Знач Приложение, Знач ЭтоПервоеПодключение = Ложь) Экспорт
	Попытка
		Пустышка = Приложение.ирПортативный;
		Значение = Приложение;
	Исключение
		Если Не ЗначениеЗаполнено(ПапкаПортативногоИР) Тогда
			...
			Если Не Файл.Существует() Тогда
				ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), "В базе нет подсистемы """ + НазваниеПодсистемы() + """ и в " + ПапкаПортативногоИР + " нет ее портативного варианта."
					...
					|Портативный - http://devtool1c.ucoz.ru/load/osnovnye/portativnye_instrumenty_razrabotchika_dlja_1s_8_2/1-1-0-6", 4000);
				ЗапуститьЗагрузкуПортативногоИР(ПапкаПортативногоИР);
				ПапкаПортативногоИР = "";
			КонецЕсли;
		КонецЕсли;
		Если ЗначениеЗаполнено(ПапкаПортативногоИР) Тогда
			...
			Если Значение <> Неопределено Тогда
				...
				Если Не Значение.Открыта() Тогда
					Попытка
						Приложение.Оповестить("ИсключениеЕслиВыполненоОбновлениеИР");
					Исключение
						ОписаниеОшибки = ОписаниеОшибки(); // Для отладки
						ЗакрытьПриложениеИР(, "для перехода на новую версию");
			...
			Иначе
				ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), "Для портативного """ + НазваниеПодсистемы() + """ нужно обычное клиентское приложение, а запущено управляемое."
					+ " Поэтому запускаем установщик расширения.", 5000);
...
Процедура ОбновитьКэшМодулей(ДобавкаТекста = "")
	...
	|Ответ ""Нет"" потребует автономный сервер. Изменения в конфигураторе будут сохранены.
...
Процедура ОбработчикОтветаЗаполнитьКэшМодулей(Значение, Параметры) Экспорт
	...
	ИмяИсполняемогоФайла = ИмяИсполняемогоФайла();
	ИДКонфигуратора = Конфигуратор.PID;
	ТурбоКонф.СохранитьЗначениеСеанса("БлокироватьЗакрытиеПриложенияИР", Истина);
	ТурбоКонф.АктивироватьОкно(Конфигуратор.ГлавноеОкно);
	ТурбоКонф.Ждать(50);
	ТурбоКонф.КонтролКлавиша(Клавиши.S);
	Если Значение = КодВозвратаДиалога.Да Тогда
		...
		Для Счетчик = 1 По МаксСчетчик Цикл
			...
			КонецЕсли;
		КонецЦикла;
		ЗакрытьПриложениеИР(ИДКонфигуратора,, Истина); // Закрываем без сохранения настроек, чтобы не потерялись настройки конфигуратора https://turboconf.ru/Tasks/8425
	КонецЕсли; 
	Если ПроцессОС = Неопределено Тогда
		ЗапуститьТолстыйКлиент(Ложь, Ложь, "/CОбновитьКэшМодулейИР;" + (Не Булево("" + Значение)), ИмяИсполняемогоФайла);
	КонецЕсли;
КонецПроцедуры

// Внешний вызов    
Процедура ПриЗакрытииКонфигуратора(Параметры)
	БлокироватьЗакрытиеПриложенияИР = ТурбоКонф.ВосстановитьЗначениеСеанса("БлокироватьЗакрытиеПриложенияИР");
	Сообщить("БлокироватьЗакрытиеПриложенияИР=" + БлокироватьЗакрытиеПриложенияИР);
	Если БлокироватьЗакрытиеПриложенияИР <> Истина Тогда
		...
		//КонецЕсли;
		ЗакрытьПриложениеИР(Параметры.PID, " после закрытия конфигуратора");
	КонецЕсли;
	ТурбоКонф.СохранитьЗначениеСеанса("БлокироватьЗакрытиеПриложенияИР", Ложь);
КонецПроцедуры
...
Процедура ЗапуститьЗагрузкуПортативногоИР(Папка)
	СоздатьКаталог(Папка);
	Файл = Новый Файл(Папка + "\ИР.zip");
	Попытка
...
Процедура ПриОткрытииОкна(Параметры)
	...
	Если ВосстановитьЗначениеБазы("РазрешенИР") = Истина Тогда 
		...
		Тогда
			ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), "После массовых изменений модулей рекомендуется выполнить команду ИР.ОбновитьКэшМодулей", 4000);
			Возврат;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПриОбновленииКэша(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;
	СтандартнаяОбработка = Ложь;
	ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), "Метаданные в приложении ИР обновляются при его перезапуске командой ИР.ЗакрытьПриложениеИР после обновления конфигурации БД", 6000);
	ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), "Модули в кэше приложения ИР обновляются автоматически при измении в окне модуля. Команда ИР.ОбновитьКэшМодулей обновляет их все.", 6000);
	ПодключениеИР = ПодключениеИР();
	Если ПодключениеИР = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), "Памятные кэши анализатора кода очищены", 3000);
...
#КонецЕсли
//
БлокироватьЗакрытиеПриложенияИР = Ложь;
