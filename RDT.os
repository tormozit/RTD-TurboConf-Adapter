...
//@script_hotkey Control+Alt+D   СоздатьОписаниеМетода Создать описание метода
...
Процедура ОбновитьСтруктуруФормы(Знач ПолеТекстаПрограммы) Экспорт
	ТекстСтруктурыФормы = ""; 
	ТурбоКонф.АктивироватьОкно(Конфигуратор.ГлавноеОкно); // Без этого часто не будет срабатывать в вызове с подключением приложения ИР
	СтароеЗначениеБуфераОбмена = ТурбоКонф.ПолучитьБуферОбмена();
	...
	Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
		Сообщить("ОшибкаПолученияФормы=" + ОписаниеОшибки);
	КонецЕсли;
...
//&ЗапроситьИнформациюОДокументе
//&ВосстанавливатьБуферОбмена
Процедура СоздатьОписаниеМетода()
	ЭтоДокумент = ЭтоДокумент();
	ПодключениеИР = ПодключениеИР(,, Ложь);
	Если ПодключениеИР = Неопределено Тогда
		Возврат;
	...
	РазобратьПозициюВТексте();
	ПолеТекстаПрограммы = ПолеТекстаПрограммыИР(ПодключениеИР, ТекстМодуля, ПозицияВТексте, ВыделенныйТекст,,, ЭтоДокумент);
	ПолеТекстаПрограммы.СоздатьОписаниеМетода(ИмяПараметра);
	ПередатьИзмененияИзПоляТекстаВОкноМодуля(ПолеТекстаПрограммы, Ложь, Ложь);
КонецПроцедуры 
...
Процедура ЗапуститьПроверкуМодулей()
	...
	Если ВыбранныеЗадания.Количество() > 0 Тогда
		Возврат;
	КонецЕсли;
	ФоновыеЗадания.Выполнить("ПроверитьИзмененныеМодули");
КонецПроцедуры
...
Процедура ПроверитьИзмененныеМодули()
	...
	Если ПолеТекстаПрограммы.Количество() > 0 Тогда
