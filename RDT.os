...
//@script_hotkey Control+OemPlus ВернутьсяИзПерехода ИР Вернуться из перехода в модуле
...
//@script_hotkey Control+B       ПерейтиВНачалоБлока ИР Перейти в начало блока
...
Процедура ПриПолученииДанныхТ9(Параметры)
	...
	Если ТурбоКонф.ВосстановитьЗначениеСеанса(КлючХранения) = Неопределено Тогда
		...
		Если ТекущаяВерсияТурбоКонф > "6.1.8837.33227" Тогда
			Если ТурбоКонф.ПолучитьЗначениеНастройки("T9AssistSmartFieldsEnabled") Тогда
				ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), "Рекомендуется отключить настройку Турбоконф ""Редактирование/ Подсказывать имена колонок""", 10000);
			КонецЕсли; 
			Если Не ТурбоКонф.ПолучитьЗначениеНастройки("T9AlwaysShowOnCtrlSpace") Тогда
				ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), "Рекомендуется включить настройку Турбоконф ""Разработчикам/ Вседа показывать подсказку Т9 вместо штатной подсказки""", 10000);
				ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), "Рекомендуется отключить настройку конфигуратора ""Модули/ Редактирование/ Автоматическая подсказка при наборе '.'""", 10000);
			КонецЕсли; 
			// Пока не работает https://turboconf.ru/Tasks/8816
			//Если ТурбоКонф.ПолучитьЗначениеНастройкиКонфигуратора(Конфигуратор.ГлавноеОкно, "PointAutoAssist") Тогда
			//	ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), "Рекомендуется отключить настройку конфигуратора ""Модули/ Редактирование/ Автоматическая подсказка при наборе '.'""", 6000);
			//КонецЕсли;
		КонецЕсли;
		ТурбоКонф.АктивироватьОкно(Конфигуратор.ОкноВызоваСкрипта);
	КонецЕсли;
...
Функция СтрокаСоединенияБазыИР(Знач ПарольИлиСтрокаСоединения = Неопределено, СтрокаСоединенияКраткая = "", Знач ЛиКраткаяСПользователем = Истина)
	...
	КонецЕсли;
	Если Не ЛиКраткаяСПользователем Тогда
		СтрокаСоединенияКраткая = Лев(СтрокаСоединенияКраткая, СтрНайти(НРег(СтрокаСоединенияКраткая), ";usr=") - 1);
	КонецЕсли;
