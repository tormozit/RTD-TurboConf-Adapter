...
Процедура ПередатьИзмененияИзПоляТекстаВОкноМодуля(Знач ПолеТекстаПрограммы) Экспорт
	Пустышка = 0;
	...
	Если НомерСтроки > 0 Тогда
		...
		Если ПолеТекстаПрограммы = Неопределено Тогда
			Если РазрешитьСсылкуИзТекущейПозиции Тогда 
				...
				ВремПозиция = 0;
				ТурбоКонф.ПолучитьТекстМодуля(Значение, ВремПозиция);
			КонецЕсли;
		Иначе
			Если РазрешитьСсылкуИзТекущейПозиции Тогда 
...
Функция ПерейтиПоСсылкеСтрокиМодуля(Знач Значение = "", Знач ИмяРасширения = "")
	...
	ТурбоКонф.ЖдатьСменуТекущегоЭлемента(СтарыйИД, 1000, Истина); // Не отключать в 8.3.24+! https://turboconf.ru/Tasks/8742
...
Процедура ПереименоватьСлово()
	...
	Попытка
		Результат = ПолеТекстаПрограммы.ПереименоватьСлово() <> Неопределено;
		ОповеститьПользователяОВозобновлении();
	Исключение
		ОписаниеОшибки = ОписаниеОшибки();
		ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), ОписаниеОшибки);
