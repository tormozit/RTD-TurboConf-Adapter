...
//@script_hotkey_ None            АвтодополнениеМодальное Автодополнение модальное
//@script_hotkey_ Control+F       НайтиСтроку Найти строку
//@script_hotkey_ Control+Enter   НажатиеCtrlENTER Нажатие Ctrl+Enter
...
Процедура НайтиСтроку() 
	// Нужна болле надежная проверка https://turboconf.ru/Tasks/9471
...
Функция НаборСловИзИР(ПолеТекстаПрограммы, ИмяНабораСлов = "")
	...
	ТаблицаСлов = ПрочитатьJSON(Чтение); // см. ПолеТекстаПрограммы.ТаблицаСловДляАдаптера()
	МоментЗапуска = ТекущаяУниверсальнаяДатаВМиллисекундах();
...
//  Шифрование	 - 	 - 
//
Процедура СохранитьЗначениеБазы(Ключ, Значение, Шифрование = Ложь)
	База = СтрокаСоединенияБазыКонфигуратора();
...
Процедура ПриЗакрытииКонфигуратора(Параметры)
	...
	КонецЕсли;
	ТурбоКонф.СохранитьЗначениеСеанса("СтрокаЗапускаОбновлениеКэшаМодулей", Неопределено);
	//ПоследниеМетоды = ВосстановитьЗначениеСеансаКонфигуратора("ПоследниеМетоды", Параметры.PID);
	//СохранитьЗначениеБазы("ПоследниеМетоды", ПоследниеМетоды); // Тут уже нет строки соединения
КонецПроцедуры
