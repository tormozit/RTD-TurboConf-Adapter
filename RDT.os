...
Функция ПолеТекстаПрограммыИзКэша() Экспорт
...
Процедура НайтиПоследнююПравкуВХранилище()
	...
	Пока Истина Цикл
		...
		КонецЕсли;
		//ТурбоКонф.Ждать(50);
		ТекстСтрокиИсторииНовый = ВыделенныйТекст(Истина);
...
Функция МодулиИР(Знач Приложение, Знач ЭтоПервоеПодключение = Ложь, выхИспользуемоеИмяФайлаПортативногоИР = "") Экспорт
	...
	Если ФлагОбновления = "ВерсияИРОбновлена" Тогда
		...
		Если ПодключениеИР <> Неопределено Тогда
			...
			КонецЕсли;
		КонецЕсли;
		ВызватьИсключение "Повторите команду для активации новой версии ИР";
	КонецЕсли;
	//! Значение = Приложение;
	Возврат Значение;
КонецФункции
...
Процедура ПриЗакрытииКонфигуратора(Параметры)
	...
	Если ПоследниеМетоды <> Неопределено Тогда
		...
		КонецЕсли;
	КонецЕсли;
	Сообщить("Завершен сеанс конфигуратора - " + СтрокаСоединенияБазы);
КонецПроцедуры
