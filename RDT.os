...
Функция ПодключениеИР(Знач ПарольИлиСтрокаСоединения = Неопределено, Знач РазобратьПозициюВТексте = Истина, Знач ВидимостьДляНовогоПодключения = Ложь, Знач ЭтоДокумент = Неопределено, выхПодключеноНоНетПодсистемы = Ложь,
	...
	УведомлениеЗапуска = ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), "Подключается приложение " + НазваниеПриложения() + ". Закрыть его можно командой ЗакрытьПриложениеИР.", 30000);
