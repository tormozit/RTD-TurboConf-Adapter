...
Функция СтрокаСоединенияБазыИР(Знач ПарольИлиСтрокаСоединения = Неопределено, СтрокаСоединенияБезПароля = "")
	...
	Если ЛиСтрокаСоединения(ПарольИлиСтрокаСоединения) Тогда
		...
		МаркерПароля = "pwd=";
		Если Найти(НРег(СтрокаСоединенияБезПароля), МаркерПароля) > 0 Тогда
			СтрокаСоединенияБезПароля = Лев(СтрокаСоединенияБезПароля, Найти(НРег(СтрокаСоединенияБезПароля), МаркерПароля) - 1);
		КонецЕсли;
	Иначе
