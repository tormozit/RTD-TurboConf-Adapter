...
Функция ПодключениеИР(Знач ПарольИлиСтрокаСоединения = Неопределено, Знач РазобратьПозициюВТексте = Истина, Знач ВидимостьДляНовогоПодключения = Ложь, Знач ЭтоДокумент = Неопределено, выхПодключеноНоНетПодсистемы = Ложь,
...
Процедура ПриВыбореЗначенияT9(Параметры, ШаблонДляВставки, ФорматироватьТекст, Отказ, СтандартнаяОбработка)
	...
	Если ПолеТекстаПрограммы = Неопределено Тогда
		Возврат;
	КонецЕсли; 
	Если Лев(Параметры.Значение, 1) = "<" Тогда
		СтандартнаяОбработка = Ложь;
		ФорматироватьТекст = Ложь;
		ШаблонДляВставки = ПолеТекстаПрограммы.Адаптер_ПриВыбореСтрокиАвтодополнения(Параметры.Значение, Параметры.ЭтоМетод, Параметры.КлючСловаря, ШаблонДляВставки);
	КонецЕсли;
КонецПроцедуры
