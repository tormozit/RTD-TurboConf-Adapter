...
Процедура ОбработчикОтветаЗаполнитьКэшМодулей(Значение, Параметры) Экспорт
	...
	КонецЦикла;
	Если ПроцессОС = Неопределено Тогда
		ЗапуститьПриложение1С("/CОбновитьКэшМодулейИР", ИмяИсполняемогоФайла, Ложь);
...
Процедура ЗапуститьТолстыйКлиент()
	...
	КонецЕсли;
	ЗапуститьПриложение1С("/DEBUG" + ДобавкаАдресОтладчика + ДобавкаПортативныйИР);
КонецПроцедуры

Процедура ЗапуститьПриложение1С(Знач ДопПараметры, Знач ИмяИсполняемогоФайла = "", Знач СПользователем = Истина)
