...
Процедура ПриПолученииДанныхТ9(Параметры)
	...
	ТекущаяСтрокаНачало = НРег(ПолеТекстаПрограммы.мТекущаяСтрокаНачало);
	// ИР Мультиметка5265263814
	Если Ложь
		Или Не Успешно
		Или (Истина
			И ЛиАвтовызов
			...
			И (Ложь
				...
				Или (Истина
					...
					И Не СтрЗаканчиваетсяНа(ТекущаяСтрокаНачало, """" + ПолеТекстаПрограммы.мКонтекст))))
	Тогда
		Возврат;
	КонецЕсли;
...
Функция ПодключениеИР(ПарольИлиСтрокаСоединения = Неопределено, РазобратьПозициюВТексте = Истина, Знач ВидимостьДляНовогоПодключения = Ложь, Знач ЭтоДокумент = Неопределено, выхПодключеноНоНетПодсистемы = Ложь) Экспорт
	...
	Попытка
		...
		Если ДлительностьПодключения > 10 Тогда
			ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), ТекстСообщения);
		Иначе
			Сообщить(ТекстСообщения);
		КонецЕсли;
...
Процедура ПредложитьПодключитьИР() Экспорт
	ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), "Хотите сразу подключить приложение " + НазваниеПриложения() + "?", 4000, ЭтотОбъект, "ПодключитьИР");
КонецПроцедуры

Процедура ПодключитьИР(Параметры)
	ПодключениеИР();
КонецПроцедуры
