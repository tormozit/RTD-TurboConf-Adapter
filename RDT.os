...
//@script_hotkey_ Escape          НажатиеEscape Нажатие Escape
//@script_hotkey_ Left            НажатиеLeft Нажатие Left
//@script_hotkey_ Right           НажатиеRight Нажатие Right
...
Процедура ПоказатьПараметрыМетода()
	//Если ОбновитьПодсказкуПоВызовуМетода1() Тогда
	//	Возврат;
	//КонецЕсли;
...
//&НажатиеLeft
//&ТолькоТекстовыеДокументы
Процедура НажатиеLeft() Экспорт
	ОбновитьПодсказкуПоВызовуМетодаСКонтролемЧастоты();
	ТурбоКонф.Клавиша(Клавиши.Left); 
КонецПроцедуры

//&НажатиеRight
//&ТолькоТекстовыеДокументы
Процедура НажатиеRight() Экспорт
	ОбновитьПодсказкуПоВызовуМетодаСКонтролемЧастоты();
...
Функция ОбновитьПодсказкуПоВызовуМетода() Экспорт
	ПодключениеИР = ПодключениеИРИзКэша();
	Если ПодключениеИР <> Неопределено Тогда
		КлючКэшаПоле = "ПолеТекстаПрограммы";
		ПолеПрограммы = ВосстановитьЗначениеСеансаКонфигуратора(КлючКэшаПоле);
		Если ПолеПрограммы <> Неопределено Тогда
			ФормаВызовМетода = ПолеПрограммы.ФормаВызовМетода();
			Если ФормаВызовМетода.Открыта() Тогда
				РазобратьПозициюВТексте(Ложь);
