...
Процедура СохранитьГраницыВыделенияДляОтмены()
	...
	АктивноеОкно = ТурбоКонф.ПолучитьАктивноеОкно(); // Вероятно 50мс
...
Процедура ЗавершитьВызовВнешнегоОкнаАсинх(Знач ВернутьФокусВОкноВызоваСкрипта = Истина, Знач ИдентификаторПроцесса = 0) Экспорт
	...
	КонецЕсли;
	//Попытка
	//	ТурбоКонф.ЗавершитьВызовВнешнегоОкнаАсинх(Окно,, ИдентификаторПроцесса); // В целях эксперимента отключил
	//Исключение
		// ниже 6.0.8819.34377
