...
//@script_hotkey None            ВыбратьТип Выбрать тип
...
//@script_hotkey Escape          НажатиеEscape Нажатие Escape
...
//@button {'Name': 'Пустышка', 'Text': '', 'Image': '', 'Anchor': 'RightTop', 'Offset': '-190,50', 'Width': 1, 'Height': 1, 'BackColor': '', 'Filter': '^(ВыражениеЪЪЪ)$' }
//@_button {'Name': 'ОтладитьОбъект', 'Text': '', 'Tooltip': 'Отладить объект', 'Image': 'Картинки\\ирОтладка.gif', 'Anchor': 'RightTop', 'Offset': '-184,50', 'Width': 16, 'Height': 16, 'BackColor': '', 'Filter': '^Выражение$' }
//@button {'Name': 'ОтладитьОбъект', 'Text': 'Отладить объект', 'Image': '', 'Anchor': 'RightTop', 'Offset': '-190,50', 'Width': 95, 'Height': 18, 'BackColor': '', 'Filter': '^Выражение$' }
//@_button {'Name': 'РедактироватьТекст', 'Text': '', 'Tooltip': 'Редактировать текст', 'Image': 'Картинки\\ирПолеТекстовогоДокумента.gif', 'Anchor': 'RightTop', 'Offset': '-202,50', 'Width': 16, 'Height': 16, 'BackColor': '', 'Filter': '^Выражение$' }
//@_button {'Name': 'РедактироватьТекст', 'Text': 'Редактировать текст', 'Image': '', 'Anchor': 'RightTop', 'Offset': '-290,50', 'Width': 115, 'Height': 18, 'BackColor': '', 'Filter': '^Выражение$' }
//@button {'Name': 'ФильтрИзСинонима', 'Text': 'Фильтр из синонима', 'Image': '', 'Anchor': 'LeftTop', 'Offset': '160,4', 'Width': 115, 'Height': 18, 'BackColor': '', 'Filter': '^Редактирование типа данных$' }
...
Процедура РедактироватьТекст()
	...
	Если Ложь
		Или ТурбоКонф.ПолучитьЗаголовокТекущегоОкна() = "Выражение" 
	Тогда
...
Процедура КонструкторМетода()
	...
	ОповеститьПользователяОПриостановке();
	ИдентификаторПроцессаИР = МодулиИР.ирКэш.ИдентификаторПроцессаОСЛкс();
	ТурбоКонф.НачатьВызовВнешнегоОкнаАсинх("^Конструктор метода.*", 5000, Конфигуратор.ГлавноеОкно, РежимОткрытияВнешнегоОкна.Диалог, Истина, ИдентификаторПроцессаИР);
	Значение = Ложь;
	// ПодключениеИР.Visible = Истина;
	Попытка
...
Функция ВыбратьЭлементСписка(Знач Элементы, Знач ТекстТекущейЯчейки = "", Знач ЛиМодально = Истина, Знач ЛиТаблица = Ложь, Знач ЧислоКолонок = Неопределено, Знач Заголовок = "") Экспорт
	...
	Если ЛиМодально Тогда
		...
		ТурбоКонф.НачатьВызовВнешнегоОкнаАсинх("(Список|Таблица|Введи).*", 3000, Конфигуратор.ГлавноеОкно, РежимОткрытияВнешнегоОкна.Диалог, Истина, ИдентификаторПроцессаИР);
	Иначе  
		ВосстановитьОкноПриложения(ПодключениеИР);
	...
	КонецПопытки;
	Если ЛиМодально Тогда
		ЗавершитьВызовВнешнегоОкнаАсинх(ЛиМодально, ИдентификаторПроцессаИР);
		ОповеститьПользователяОВозобновлении();
		ПодключениеИР.Visible = Ложь;
	Иначе
...
Процедура ОтладитьОбъект()
	...
	Если Не Успех Тогда
		...
		ЗавершитьВызовВнешнегоОкнаАсинх(Ложь, ИдентификаторПроцесса);
		//Успех = Истина;
	КонецЕсли;           
	Если Не Успех Тогда
...
Процедура ПриПолученииДанныхТ9(Параметры)
	...
	Если ТурбоКонф.ВосстановитьЗначениеСеанса(КлючХранения) = Неопределено Тогда
		...
		ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), "Размер статистики выбора - " + РазмерСтатистикиВыбора + ". Актуальность кэша модулей - " + ДатаОбновленияКэшаМодулей + "." + ДобавкаТекста, 5000,
...
// Внешний вызов
// см. СобытияМодуль.ПослеВставкиЗначенияT9
