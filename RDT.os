...
//@_turbobutton 1 // Отображать команды скрипта в меню Турбокнопки (в текущей строке модуля)
...
Процедура ПерейтиКОпределению()
	...
	Если ПродолжатьОбработку Тогда
		...
		// 	ВосстановитьОкноПриложения(ПодключениеИР); 
		// 	ПродолжатьОбработку = Ложь;
		ИначеЕсли СтрНачинаетсяС(Представление, "Метод прямой") Тогда
		...
		Тогда 
			...
			Если ЕстьКнопкаОбновитьДинамически Тогда 
				...
				Если УспехУстановкиФокуса Тогда
					ТурбоКонф.Клавиша(Клавиши.Enter);
					Попытка
						ТурбоКонф.ЖдатьЗакрытияОкна(АктивноеОкно, ЗаголовокОкна, 5000);
						Дождались = Истина; 
					Исключение
						Дождались = Ложь;
					КонецПопытки;
					Если Дождались Тогда
						ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), "Конфигурация БД обновлена динамически из-за подключенного приложения ИР. Подключить новое?", 5000, 
							ЭтотОбъект, "ЗапуститьПереподключениеПриложенияИР");
					Иначе
						ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), "Конфигурация БД обновляется динамически из-за подключенного приложения ИР", 3000); 
					КонецЕсли;
				КонецЕсли;
			Иначе 
