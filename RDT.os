...
//@script_hotkey Control+Alt+F12 НайтиСсылкиНаСлово ИР Найти ссылки на слово
//@script_hotkey None            ОбновитьКэшМодулей ИР Обновить кэш модулей
//@script_hotkey Alt+Q           ОткрытьОбъектМетаданных ИР Открыть объект метаданных
...
//@script_hotkey None            ТочкиОстановаИзРезультатовПоиска ИР Точки останова из результатов поиска
...
Процедура КопироватьСсылку()
	...
	Иначе 
		...
		Ссылка = ПолеТекстаПрограммы.СсылкаСтрокиМодуля(, Истина) + Символы.ПС + ПолеТекстаПрограммы.СсылкаСтрокиМодуля(, Ложь);
	КонецЕсли;
	...
	Если ПродолжатьОбработку Тогда
		...
		Если Значение = Истина Тогда
			...
			Иначе
				ВыделениеОдномерное = ПолеТекстаПрограммы.ПолеТекста.ВыделениеОдномерное();
				ТурбоКонф.ВыделитьТекст(ВыделениеОдномерное.Начало - 1, ВыделениеОдномерное.Конец - 1);
				ДлинаВыделения = ВыделениеОдномерное.Конец - ВыделениеОдномерное.Начало;
			КонецЕсли;
...
Функция ПерейтиПоСсылкеСтрокиМодуля(Знач Значение = "", Знач ИмяРасширения = "")
	...
	Иначе
		...
		Если ЗначениеЗаполнено(СтруктураСсылки.Метод) Тогда
			...
			Тогда
				ТурбоКонф.Ждать(20); // Иначе может вернуться пустой текст модуля
				Попытка
				...
				Если ОписаниеМетода <> Неопределено Тогда
					НомерСтроки = ПолеТекстаПрограммы.НомерСтрокиИзПозиции(ОписаниеМетода.ПозицияОпределения) + СтруктураСсылки.СмещениеСтрокиМетода;
					//Если ТурбоКонф.ПолучитьВерсиюAPI() < 6 Тогда
						// Так всегда будет в центре по высоте
						ТурбоКонф.ПерейтиВПозицию(СтруктураСсылки.НомерКолонки, НомерСтроки); 
					//Иначе
					//	// Так часто будет в самом низу поля
					//	Позиция = ПолеТекстаПрограммы.ПолеТекста.ПозицияВПолеТекстаПоНомеруСтрокиИКолонки(НомерСтроки, СтруктураСсылки.НомерКолонки) - 1;
...
Процедура НайтиСсылкиНаСлово()
	...
	КонецЕсли;
	Если Не ПолеТекстаПрограммы.ОткрытьПоискВызововСлова(Ссылка, Истина, Ложь) Тогда 
		Возврат;
	КонецЕсли;
	ВосстановитьОкноПриложения(ПодключениеИР);
