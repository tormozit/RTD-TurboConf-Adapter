...
//@script_hotkey Control+OemOpenBrackets ПерейтиКГраницеКонструкции Перейти к границе конструкции 1
//@script_hotkey Control+OemCloseBrackets ПерейтиКГраницеКонструкции Перейти к границе конструкции 2
...
//! Перем ТипЭлемента; // ПеречислениеСсылка.ТипЭлементаИнтерфейса
//! Перем ФоновыеЗадания; // ОбработкаОбъект.ФоновыеЗадания
		...
		Если Значение.Количество() = 0 И ЗначениеЗаполнено(ТекстМодуля) Тогда
			...
			Если Значение = Неопределено Тогда
				...
				Если Вхождения.Количество() Тогда
					Подгруппы = Вхождения.Получить(0).Подгруппы;
					ПространствоИмен = Подгруппы.Получить(1);
					...
					Если Пакет <> Неопределено Тогда
						Значение = "ПакетXDTO." + Пакет.Имя + "." + Подгруппы.Получить(0);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			Если Значение = Неопределено Тогда
...
Процедура ПоказатьПараметрыМетода()
	//ОткрытьПараметрыМетодаВФоне();
	ПодключениеИР = ПодключениеИР(,, Ложь);
	Если ПодключениеИР = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ВосстановитьОкноПриложения(ПодключениеИР);
	ФоновыеЗадания.Выполнить(ЭтотОбъект, "ОткрытьПараметрыМетодаВФоне",, Истина);
	Возврат;
...
Функция ОбновитьПараметрыМетодаИВернутьФокус(Знач ВернутьФокусВКонфигуратор = Истина) Экспорт
	//Если ВернутьФокусВКонфигуратор Тогда
	//	ТурбоКонф.ПоказатьВсплывающееУведомление("тест", "Возврат фокуса", 500);
	//КонецЕсли;
	Пока Истина Цикл
		...
		Если ВернутьФокусВКонфигуратор Тогда
			...
			//ТурбоКонф.ПоказатьВсплывающееУведомление("тест", "Возврат фокуса 1", 500);
			//ТурбоКонф.Ждать(100);
			//ТурбоКонф.АктивироватьОкно(Конфигуратор.ГлавноеОкно);
		...
		КонецЕсли;
		Если ВернутьФокусВКонфигуратор Тогда
			//ТурбоКонф.ПоказатьВсплывающееУведомление("тест", "Возврат фокуса 2", 500);
			ТурбоКонф.Ждать(50);
		КонецЕсли;
		...
		Если ВернутьФокусВКонфигуратор Тогда
			//ТурбоКонф.ПоказатьВсплывающееУведомление("тест", "Вернули фокус", 500);
			//ТурбоКонф.Ждать(100);
			ТурбоКонф.АктивироватьОкно(Конфигуратор.ГлавноеОкно);
			ВернутьФокусВКонфигуратор = Ложь;
		КонецЕсли;
...
Процедура ОткрытьПараметрыМетодаВФоне() Экспорт
...
Процедура НажатиеEscape() Экспорт
	...
	Если ПодключениеИР <> Неопределено Тогда
		КлючКэша = "ПолеТекстаПрограммы";
		ПолеПрограммы = ВосстановитьЗначениеСеансаКонфигуратора(КлючКэша);
		Если ПолеПрограммы <> Неопределено Тогда
			ФормаВызовМетода = ПолеПрограммы.ФормаВызовМетода();
...
Процедура ОбновитьПодсказкуПоВызовуМетодаСКонтролемЧастоты() Экспорт
...
Функция ОбновитьПодсказкуПоВызовуМетода() Экспорт
	ПодключениеИР = ПодключениеИРИзКэша();
	Если ПодключениеИР <> Неопределено Тогда
		ПолеПрограммы = ВосстановитьЗначениеСеансаКонфигуратора("ПолеТекстаПрограммы");
		Если ПолеПрограммы <> Неопределено Тогда
			ФормаВызовМетода = ПолеПрограммы.ФормаВызовМетода();
			Если ФормаВызовМетода.Открыта() Тогда
				РазобратьПозициюВТексте(Ложь);
...
#Если Сервер И Не Сервер Тогда
	...
	ФоновыеЗадания = Обработки.ФоновыеЗадания.Создать();
#КонецЕсли
