...
Процедура ПерейтиКОпределению()
	...
	Если ФайлБуфера.Существует() Тогда
		...
		ЗначениеИзБуфера = ТекстовыйДокумент.ПолучитьТекст();
	...
	Иначе
		...
		Если Значение.Количество() = 0 Тогда
			...
			ПолеТекстаПрограммы = ПолеТекстаПрограммыИР(ПодключениеИР, ТекстМодуля, СтрДлина(ТекстМодуля), ВыделенныйТекст,,, Ложь, Истина);
			Если ПолеТекстаПрограммы <> Неопределено Тогда
				ПолеТекстаПрограммы.КончитьОбработкуКоманды();
				Значение = ПолеТекстаПрограммы.ПерейтиКОпределению(,, Ложь);
			КонецЕсли;
...
Процедура КопироватьВсеСтрокиВБуфер()
	...
	Если ЗначениеЗаполнено(ТипЭлементаИнтерфейса) И ТипЭлементаИнтерфейса <> ТипЭлемента.Неизвестный Тогда
...
Процедура ПриПолученииДанныхТ9(Параметры)
	...
	Если ПолеТекстаПрограммы = Неопределено Тогда
		ПолеТекстаПрограммы = ПолеТекстаПрограммыИР(ПодключениеИР, ТекстМодуля, ПозицияВТексте,,,, ЭтоДокумент,, Истина, ИдентификаторПоля, ИмяКонфигурации, ОбновленаСтруктураФормы);
		Если ПолеТекстаПрограммы = Неопределено Тогда
			Возврат;
		КонецЕсли;	
...
Функция ПолеТекстаПрограммыИР(ПодключениеИР, ТекстМодуля, ПозицияВТексте, ВыделенныйТекст = "", ПереходитьВоВложенныйКонтекст = Истина, Знач КакВызовМетода = Неопределено,
	Знач ЭтоДокумент = Истина, Знач СИменемКонфигурации = Ложь, Знач ЭтоАвтодополнение = Ложь, Знач ИдентификаторПоля = Неопределено, Знач ИмяКонфигурации = "", выхОбновленаСтруктураФормы = Ложь,
	выхИмяРасширения = "")
	...
	Тогда
		...
		ПолноеИмяМодуля = ИмяМодуляДокумента(ПодключениеИР, ТекстМодуля, ИмяКонфигурации,, Не ЭтоАвтодополнение, выхИмяРасширения);
...
Процедура ПередатьИзмененияМодулей(Знач ПодключениеИР, Знач ПолеТекстаПрограммы = Неопределено) Экспорт
	Попытка
		ДокументыМодулей = ТурбоКонф.ПолучитьМодифицированныеДокументы();
	Исключение
		// 6.0.8736.33837+
	...
	Если ДокументыМодулей.Количество() > 0 Тогда
		...
		Для Каждого Документ Из ДокументыМодулей Цикл
			...
			Сообщить("Синхронизация изменений модуля "+ ПолноеИмяМодуля + " " + (ТекущаяУниверсальнаяДатаВМиллисекундах() - МоментЗапуска) + " мс");
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
	...
	Исключение
		...
		Если ЗначениеЗаполнено(ПапкаПортативногоИР) Тогда
			...
			Если Значение <> Неопределено Тогда
				...
				КонецЕсли;
			Иначе
				ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), "Для портативного варианта """ + НазваниеПодсистемы() + """ нужно обычное клиентское приложение, а запущено управляемое."
...
Процедура ПриОткрытииОкна(Параметры)
	...
	Если Истина
		...
		И Ложь  // Жду способа проверить недоступность кнопки https://turboconf.ru/Tasks/8884
	Тогда
...
Процедура ПриОбновленииКэша(Отказ, СтандартнаяОбработка)
	Если ПроверитьРазрешениеИР(Отказ) Тогда
		...
		КонецЕсли;
		ПолеТекстаПрограммы = ПолеТекстаПрограммыИР(ПодключениеИР, ТекстМодуля, ПозицияВТексте, ВыделенныйТекст); 
		ПолеТекстаПрограммы.мПлатформа.СброситьКэшиТиповВыраженийМодулей();
		ТурбоКонф.ПоказатьВсплывающееУведомление(НазваниеСкрипта(), "Кэш типов выражений модулей очищен", 3000);
